...
public class ClinicaManager {
    ...
    public boolean marcarAgendamento(Paciente paciente, Dentista dentista, Procedimento procedimento, LocalDateTime dataHora, String sala) {
        ...
        if (paciente.getPossuiPagamentoPendente()) {
            System.out.println("Erro: main.java.model.Paciente possui pagamentos pendentes. main.java.model.Agendamento bloqueado.");
            return false;
        }

        // REQ23: Verificar disponibilidade
        if (!verificarDisponibilidadeDentista(dentista, dataHora, procedimento.getDuracaoEmMinutos())) {
            System.out.println("Erro: main.java.model.Dentista indisponível no horário solicitado.");
            return false;
        }

        // Cria e salva
        Agendamento novoAgendamento = new Agendamento(paciente, dentista, procedimento, dataHora, sala);
        // Se precisar associar o procedimento ao agendamento, sua classe main.java.model.Agendamento precisa de um campo 'procedimento'.
        // Baseado no seu arquivo main.java.model.Agendamento.java, não vi o campo procedimento no construtor,
    ...
    public void registrarPagamento(Pagamento pagamento) {
        ...
        System.out.println("main.java.model.Pagamento registrado: " + pagamento.getValor());
    }
    ...
    private boolean verificarDisponibilidadeDentista(Dentista dentista, LocalDateTime dataHora, int duracaoMinutos) {
        // 1. Verificar folgas e ausências do objeto main.java.model.Dentista
        ...
        for (Agendamento a : todosAgendamentos) {
            ...
            if (a.getDentista().getCpf().equals(dentista.getCpf()) && a.getStatus() != StatusAgendamento.CANCELADO) {
                ...
                // Como main.java.model.Agendamento.java não tem o procedimento direto no código que você mandou,
                // vou assumir uma duração padrão de 30 min ou você deve adicionar 'main.java.model.Procedimento' em 'main.java.model.Agendamento'.
